...
class ComputationalEngineEnvironment(Environment):
    """
    OpenEnv environment for computational engine simulation
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.state = ComputationalEngineState()
        self.engine_designer = ComputationalEngine(
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> tuple[Observation, dict]:
        ...
        self.state = ComputationalEngineState()
        self.state.max_steps = self.config.get("max_steps", 100)
    ...
    def step(self, action: Action) -> tuple[Observation, float, bool, bool, dict]:
        ...
        if not isinstance(action, ComputationalEngineAction):
            # Convert generic action to our specific type
        ...
        self.state.engine_type = action.engine_type
        ...
        if not self.state.engine_generated:
            engine_field = self._generate_engine(action.engine_type)
            self.state.voxel_field = engine_field
            self.state.engine_generated = True
            
            # Calculate initial metrics
            surface_voxels = engine_field.extract_surface(threshold=1.0)
            self.state.engine_metrics = {
            ...
            reward += 1.0  # Reward for successful generation
        
        # Run simulation step
        if self.state.simulation is None:
            self.state.simulation = EngineSimulation()
            self.state.simulation.add_engine(
                self.state.voxel_field, 
                engine_type=self.state.engine_type
            )
        
        # Progress simulation
        progress = min(self.state.current_step / self.state.max_steps, 1.0)
        self.state.simulation.simulate_assembly(steps=1)  # Single step
        
        if progress >= 1.0:
            done = True
            reward += 2.0  # Completion reward
            self.state.assembly_complete = True
        
        # Create observation
        observation = ComputationalEngineObservation(
            engine_generated=self.state.engine_generated,
            engine_type=self.state.engine_type,
            voxel_dimensions=list(self.state.voxel_field.size) if self.state.voxel_field else [],
            surface_voxel_count=self.state.engine_metrics.get("surface_voxel_count", 0),
            simulation_progress=progress,
            assembly_complete=self.state.assembly_complete,
            engine_metrics=self.state.engine_metrics,
    ...
    def render(self):
        ...
        if self.state.voxel_field:
            print(f"Engine Voxel Field: {self.state.voxel_field.size}")
            if self.state.engine_metrics:
                print(f"Surface voxels: {self.state.engine_metrics.get('surface_voxel_count', 0)}")
                print(f"Estimated volume: {self.state.engine_metrics.get('estimated_volume_mm3', 0):.2f} mm³")
